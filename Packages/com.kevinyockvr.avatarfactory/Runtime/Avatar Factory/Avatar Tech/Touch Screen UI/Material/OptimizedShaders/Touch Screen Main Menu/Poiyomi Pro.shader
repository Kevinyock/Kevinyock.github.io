Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/7c2d058ecc7d41b4b82a2736f413311b"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 9.0.α09</color>", Float) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyPositionText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(0,0.25,0,0), (0.0 /*_TextPositionRotation*/), float4(0.5,0.1,1,1), uv);
				...
				globalTextEmission += poiThemeColor(poiMods, float4(1,0,1,1).rgb, (0.0 /*_TextPositionColorThemeIndex*/)) * opacity * (1.0 /*_TextPositionEmissionStrength*/);
			}
			void ApplyTimeText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(0,0.15,0,0), (0.0 /*_TextTimeRotation*/), float4(0.25,0.1,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				globalTextEmission += poiThemeColor(poiMods, float4(1,0,1,1).rgb, (0.0 /*_TextTimeColorThemeIndex*/)) * opacity * (1.0 /*_TextTimeEmissionStrength*/);
			}
			void ApplyFPSText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(0,0.35,0,0), (0.0 /*_TextFPSRotation*/), float4(0.2,0.1,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
				float opacity = clamp(sigDist + 0.5, 0, 1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, poiThemeColor(poiMods, float4(1,0,0,1).rgb, (0.0 /*_TextFPSColorThemeIndex*/)), opacity * float4(1,0,0,1).a);
				globalTextEmission += poiThemeColor(poiMods, float4(1,0,0,1).rgb, (0.0 /*_TextFPSColorThemeIndex*/)) * opacity * (1.0 /*_TextFPSEmissionStrength*/);
			}
			void ApplyNumericText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				float charSign = (9999.0 /*_TextNumericValue*/) >= 0 ? ASCII_SPACE : ASCII_NEGATIVE;
				NumericArray[arrayIndex] = charSign;						//First character is always the sign
				arrayIndex++;
				if (wholeDigits > 0)
				{
					wholeNumber = uint(glsl_mod(abs((9999.0 /*_TextNumericValue*/)), pow(10, wholeDigits)));
				...
				if (decimalDigits > 0)
				{
					...
					arrayIndex++;
					totalCharacters++;
					decimalNumber = uint(frac(abs((9999.0 /*_TextNumericValue*/))) * pow(10.00001, decimalDigits));    // Isolate the decimal number
				...
				uv = TransformUV(float4(-0.025,0.05,0,0), (0.0 /*_TextNumericRotation*/), float4(0.25,0.1,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				globalTextEmission += poiThemeColor(poiMods, float4(1,1,1,1).rgb, (0.0 /*_TextNumericColorThemeIndex*/)) * opacity * (1.0 /*_TextNumericEmissionStrength*/);
			}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.indirectColor = (1.0 /*_LightingAdditiveLimited*/) ? min((1.0 /*_LightingAdditiveLimit*/), poiLight.indirectColor) : poiLight.indirectColor;
				#endif
